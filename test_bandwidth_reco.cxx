...
#include <algorithm>
#include <cstring>
#include <cmath> // 🌟 표준편차 계산을 위한 sqrt 함수 포함
...
class EtrocChannelAnalyzer {
    ...
    RecoStats s_;
    uint64_t last_processed_word_ = 0xFFFFFFFFFFFFFFFFULL; // 🌟 디바운스를 위한 이전 단어 기억 변수
...
public:
    ...
    void process_word(uint64_t w) {
        s_.total_frames++;
        // 🌟 [핵심 소프트웨어 방어막] Hardware Stutter(복제 버그) 원천 차단!
        if (w == last_processed_word_) {
            // 방금 처리한 단어와 토씨 하나 안 틀리고 100% 똑같다면, 
            // 이는 ETROC2가 보낸 것이 아니라 FPGA 클럭 엇박자로 복제된 쓰레기입니다.
            return; // 깔끔하게 무시!
        }
        last_processed_word_ = w; // 현재 단어 기억
...
int main() {
    ...
    for (const string& fn : files) {
        ...
        auto process_dpram_word = [&](uint32_t w) {
            ...
            // 1. 상태 레지스터(Drop / Lap)는 실제 Payload 데이터가 아니므로 스킵
            if ((w >> 31) == 0) return; 

            // 2. 데이터 워드 (MSB == 1), 최상위 3비트로 채널 분기
            ...
            switch (channel_and_part) {
                ...
                case 0b101: // Right High (16 bits)
                    if (right_low_ready) {
                        ...
                        // High가 짝 없이 들어온 경우 에러 플래그 처리 후 해당 채널만 동기화 리셋
