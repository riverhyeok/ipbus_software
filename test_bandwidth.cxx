...
// =====================================================================
// 1. I2C Command Generator (비트 시프트 완벽 적용)
// =====================================================================
...
// =====================================================================
// =====================================================================
// 2. I2C Write / Read Functions (IPbus 패킷 증발 방어 & 자동 재시도 적용)
// =====================================================================
bool i2c_write_reg(uhal::HwInterface& hw, uint16_t reg_addr, uint8_t data) {
    ...
    for(int i = 0; i < 3; i++) {
        try {
            ...
            usleep(2000); // UDP 패킷 씹힘! 2ms 쉬고 재시도
        }
    }
    if(!dispatched) return false; // 3번 다 실패하면 I2C 실패로 처리
    ...
    do {
        try {
            ...
            // 읽는 중에 UDP 패킷이 씹히면? 당황하지 않고 2ms 대기 후 루프 재진입 (다시 읽기)
            usleep(2000);
            continue; 
        }
...
void configure_readout_fpga(uhal::HwInterface& hw) {
    ...
    while(true) {
        ...
        uint32_t serialzer_alignment_reset = (val >> 18) & 0x1; // 주석 해제 및 변수 활성화!
        ...
        if (sel >= 1 && sel <= 7) {
            ...
            else if (sel == 7) val = (val & ~(0x1  << 18)) | ((in_val & 0x1)  << 18); // 18번째 비트에 1비트 마스킹 쓰기 로직 추가
        } 
        else if (sel == 8) {
            try {
